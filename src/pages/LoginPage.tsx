...
export default function LoginPage() {
  ...
  return (
    <main className="min-h-screen bg-white text-violet-950">
      <div className="mx-auto grid min-h-screen max-w-7xl items-center gap-12 px-6 py-12 lg:grid-cols-[1.05fr_0.95fr] lg:px-10">
        <section className="animate-fade-up">
          <div className="mb-16 flex items-center gap-3">
            <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-violet-950 text-sm font-medium text-white">EO</span>
            <div>
              <p className="text-base font-medium">EventOS</p>
              <p className="text-sm text-violet-950/55">Gestão premium para eventos</p>
            </div>
          </div>
          ...
          <p className="mt-6 max-w-2xl text-base leading-8 text-violet-950/66">
          ...
          <div className="mt-10 grid max-w-2xl gap-3 sm:grid-cols-3">
            {highlights.map(([Icon, text]) => (
              <div key={text} className="rounded-xl border border-violet-200 bg-violet-50 p-4">
                <Icon className="h-5 w-5 text-violet-800" />
                <p className="mt-4 text-sm leading-5 text-violet-950/72">{text}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="flex justify-center lg:justify-end">
          <Card className="w-full max-w-md animate-scale-in p-8">
            ...
            <p className="mt-2 text-sm leading-6 text-violet-950/60">Acesse a área administrativa da sua empresa.</p>
            <form className="mt-7 space-y-5" onSubmit={form.handleSubmit(onSubmit)}>
              <div>
                <Label>E-mail</Label>
                <Input type="email" {...form.register("email")} />
                <p className="mt-1 text-xs text-fuchsia-700">{form.formState.errors.email?.message}</p>
              </div>
              <div>
                <Label>Senha</Label>
                <Input type="password" {...form.register("password")} />
                <p className="mt-1 text-xs text-fuchsia-700">{form.formState.errors.password?.message}</p>
              </div>
              {error && <p className="rounded-md bg-fuchsia-50 px-4 py-3 text-sm text-fuchsia-800">{error}</p>}
              <Button className="w-full" disabled={form.formState.isSubmitting}>
                {form.formState.isSubmitting ? "Entrando..." : "Entrar"}
                <ArrowRight className="h-4 w-4" />
              </Button>
            </form>
            <p className="mt-6 text-center text-sm text-violet-950/60">
              Ainda não tem conta?{" "}
              <Link to="/cadastro" className="font-medium text-violet-800 underline decoration-violet-300 underline-offset-4">
                Criar empresa
              </Link>
            </p>
          </Card>
